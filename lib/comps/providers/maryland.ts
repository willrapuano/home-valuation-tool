...
async function esriQueryOnce(url: string, params: Record<string, string>): Promise<EsriFeature[]> {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`Maryland iMAP returned HTTP ${res.status}`);
    ...
    try {
      ...
      throw new Error("Maryland iMAP returned a non-JSON response");
    }
    if (data?.error) {
      // A malformed query is our bug and will fail identically on retry; only
      // transport-level failures are worth attempting again.
      const err = new Error(`Maryland iMAP error: ${data.error.message ?? "unknown"}`);
    ...
    if ((err as Error)?.name === "AbortError") {
      throw new Error(`Maryland iMAP request timed out after ${TIMEOUT_MS}ms`);
    }
    throw err;
  } finally {
    clearTimeout(timer);
 ...
 *
 * The service throttles under sustained load: a burst of queries starts
 * returning timeouts while a single query against the same layer answers in
 * under a second. Without a retry that surfaces to the user as "we can't value
 * your home" for what is a momentary condition.
 */
